...
class ObstacleSmoother:
    ...
    def _coloringgap(self, mesh1d, s):
        '''Compute c which is the gap between nodes with the same color.
    ...
    def _newtonlusweep(self, mesh1d, s, ella, r,
                       eps=1.0, dump=False, band=1):
        '''Do one in-place projected Newton step on s(x), with an approximated
        Jacobian using coloring, and using LU to solve the linear step
        equations.  The approximated Jacobian is band-limited to [-band,band]
        relative to the diagonal; band = 1 gives tridiagonal.  We require
        band + 2 <= c where c is the gap generated by coloring.'''
        ...
        A.setFromOptions()
        A.setUp()
        for i in range(mA):  #FIXME serial
            # FIXME
