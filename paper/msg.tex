...
In this section we propose solvers for \eqref{eq:fesigpweakform}.  We describe certain projected and nonlinear iterations, variants of the Gauss-Seidel and Jacobi methods, which are suitable for VIs \cite{KinderlehrerStampacchia1980}.  While these iterations are adequate smoothers in a multilevel method (next section), they converge slowly if used as single-level methods.

Given a current surface elevation $s^h\in \mathcal{K}^h$, these smoothers sweep through the vertices ($P_1$ nodes) in $\mathcal{T}$, in a fixed order, solving one-dimensional VI problems and then updating $s^h$.   Denote an arbitrary vertex of $\mathcal{T}$ by $(x_i,y_i)$ and let $\psi_i \in \mathcal{V}^h$ be the corresponding hat function, with $\psi_i(x_j,y_j)=\delta_{ij}$ \cite{Elmanetal2014}.  Also let $\mathcal{D}_+^h = \{t^h \in \mathcal{V}^h \,:\, t^h \ge 0\}$.  Note that, because $\psi_i \in \mathcal{D}_+^h$, $s^h + c \psi_i \in \mathcal{K}^h$ if and only if $c\ge \omega_i^{s^h} = b^h(x_i,y_i) - s^h(x_i,y_i)$.

The one-dimensional VI at node $i$ is derived from \eqref{eq:fesigpweakform}.  It finds $c \ge \omega_i^{s^h}$ such that
\begin{equation}
F^h(s_c)[r_\gamma - s_c] \ge \ip{a}{r_\gamma - s_c} \label{eq:fepointwiseviEARLY}
\end{equation}
for all $\gamma \ge \omega_i^{s^h}$, with $r_\gamma = s^h+\gamma \psi_i$ and $s_c = s^h+c \psi_i$.  Note \eqref{eq:fepointwiseviEARLY} simplifies substantially.  Let
\begin{equation}
\rho_i(s^h; c) = F^h(s^h+c\psi_i)[\psi_i] - \ip{a}{\psi_i} \label{eq:ferhoi}
\end{equation}
be the point-wise residual function.  Then \eqref{eq:fepointwiseviEARLY} is equivalent
\begin{equation}
(\gamma - c) \,\rho_i(s^h; c) \ge 0. \label{eq:fepointwisevi}
\end{equation}
Supposing \eqref{eq:fepointwisevi} is well-posed, after solving it for $c$ the surface elevation $s^h$ is then updated $s^h \gets s^h + c \psi_i$ by the smoother.  Observe that admissibility is preserved because the updated surface elevation is also in $\mathcal{K}^h$.

If each pointwise update is done immediately then the smoother is of Gauss-Seidel (GS) type, sometimes called a serial or multiplicative iteration. By contrast, the (parallel, additive) Jacobi smoother computes solutions at all nodes $i$ before doing all the updates.  In linear problems it is known that Gauss-Seidel generally converges in about half as many iterations \cite{Greenbaum1997}, but here there is a more glaring difference between the two iterations.  The GS iteration would be extraordinarily expensive because a new Glen-Stokes solution would be required at every node of $\mathcal{T}$, so we will only implement the Jacobi version.

If $\rho_i(s^h; c)$ were linear and increasing in $c$ then the solution to \eqref{eq:fepointwisevi} would be given by a simple formula \cite{GraeserKornhuber2009}, namely $c = \max\left\{-\beta/\alpha, \omega_i^{s^h}\right\}$ if $\rho_i(s^h; c) = \alpha c + \beta$ with $\alpha > 0$.  In fact $\rho_i(s^h; c)$ is nonlinear, but the simplest approximation scheme is to linearize $\rho_i(s^h; c)$, that is, to do one projected Newton step for the one-dimensional VI problem \eqref{eq:fepointwisevi} at each node.

To implement this strategy we approximate
\begin{equation}
\rho_i(s^h; c) \approx \rho_i(s^h; 0) + c\, (F^h)'(s^h)[\psi_i,\psi_i]. \label{eq:rhoapprox}
\end{equation}
If we partition the support of $\psi_i$ into where there is ice and not,
\begin{equation}
\theta_i = \{\psi_i > 0\} \cap \{s^h > b^h\}, \qquad {\hat\theta}_i = \{\psi_i > 0\} \setminus \theta_i,  \label{eq:thetasupport}
\end{equation}
then by \eqref{eq:sigpfunctionalderiv}, \eqref{eq:idoderiv}, \eqref{eq:differencecases}, and \eqref{eq:ferhoi} we may compute with local integrals
\begin{equation}
\rho_i(s^h; 0) = F^h(s^h)[\psi_i] - \ip{a}{\psi_i} = - \int_{\theta_i} \bu|_{s^h} \cdot \bn_{s^h}\, \psi_i\,dx\,dy - \ip{a}{\psi_i}  \label{eq:rhozero}
\end{equation}
and
\begin{align}
(F^h)'(s^h)[\psi_i,\psi_i] &= - \int_{\theta_i} \lim_{\eps\to 0^+} \frac{(\bu|_{s^h+\eps\psi_i} - \bu|_{s^h}) \cdot \bn_{s^h+\eps\psi_i}}{\eps} \psi_i\,dx\,dy \label{eq:rholocalderiv} \\
  &\qquad - \int_{{\hat\theta}_i} \lim_{\eps\to 0^+} \frac{\bu|_{s^h+\eps\psi_i} \cdot \bn_{s^h+\eps\psi_i}}{\eps} \psi_i\,dx\,dy.  \notag
\end{align}

If $\alpha=(F^h)'(s^h)[\psi_i,\psi_i]$ is positive then the solution of \eqref{eq:fepointwisevi} is
\begin{equation}
  c = \max\left\{-\frac{\rho_i(s^h; 0)}{\alpha}, \omega_i^{s^h}\right\}, \label{eq:fepointwisevisolution}
\end{equation}
which generalizes a well-known formula for the classical obstacle problem \cite[formula (4.4), for example]{GraeserKornhuber2009}.  However, if $\alpha\le 0$ then the model has degenerated in the sense that the linearized operator is not acting elliptically with respect to that perturbation.  In that case we make no change to $s^h$; we set $c=0$.  While one can prove in the shallow, flat-bed model that the analogous $\alpha$ alpha quantity is positive on the icy part of $\Omega$ \cite{JouvetBueler2012}, which follows since the equation is elliptic, the full Glen-Stokes model allows no such guarantee to our knowledge.

We will approximate the limits in \eqref{eq:rhozero} and \eqref{eq:rholocalderiv} by using the obvious difference quotients, i.e.~by removing ``$\lim_{\eps\to 0^+}$'' and choosing $\eps$ to be small, and we set $\eps$ to one meter in computations.  However, we need to make further approximations to the computation of the divisor $\alpha$, i.e.~in \eqref{eq:rholocalderiv}, if we want a practical solver.

Note that $\bu|_{z}$ denotes the surface velocity of the solution to the Glen-Stokes problem \eqref{eq:glenstokesweak} on the domain $\Lambda_{z}$, using an extruded mesh (Section \ref{sec:fe}).  Thus $\bu|_{s^h}$ and $\bu|_{s^h+\eps\psi_i}$ are the results of separate Glen-Stokes solves, over $\Lambda_{s^h}$ and $\Lambda_{s^h+\eps\psi_i}$, respectively.  Clearly, each smoother application, a sweep over all nodes in $\mathcal{T}$, will require at least one solution of the Glen-Stokes problem \eqref{eq:glenstokesweak}, namely solving with the geometry determined by the current surface elevation $s^h$.  However, formulas \eqref{eq:rhozero} and \eqref{eq:rholocalderiv} as stated are prohibitively expensive; a Stokes solve is required  for each mesh node.

Thus we seek a less expensive way to estimate the surface velocity for a perturbed surface elevation with a small ``bump'' $\eps\psi_i$.  The nonlinearities in this Glen-Stokes problem should be unimportant, and it would seem that the most important change to the velocity and pressure fields from a small bump perturbation is through the perturbed gravitational load applied to the linearized problem around the state $s^h$.  (Versus the additional effect of a perturbed normal direction on the surface, for example.)

In fact, let us fix a current surface elevation $s^h$ and assume that problem \eqref{eq:glenstokesweak} for $\Lambda_{s^h}$ yields solution $\bu^h,p^h$ at the convergence of its (Newton) iteration.  Noting that we are using a stable mixed space for this Glen-Stokes problem (Section \ref{sec:fe}), the final step in the iteration can be regarded as providing the solution of a linear system
\begin{equation}
