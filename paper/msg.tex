...
\end{equation}
which generalizes a well-known formula for the classical obstacle problem \cite[formula (4.4), for example]{GraeserKornhuber2009}.  However, if $\alpha\le 0$ then the model has degenerated in the sense that the linearized operator is not acting elliptically with respect to that perturbation.   In that case we make no change to $s^h$; we set $c=0$.  Because the interior PDE in the shallow, flat-bed model is elliptic, one can prove that the analogous $\alpha$ is positive on the icy part of $\Omega$ \cite{JouvetBueler2012}, but the full Glen-Stokes model here allows no such guarantee to our knowledge.
