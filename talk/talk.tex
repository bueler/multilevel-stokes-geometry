...
\begin{align*}
\frac{\partial s}{\partial t} - \bu|_s \cdot \bn_s &= a & &\text{\emph{evolving}} \\
\bu|_s \cdot \bn_s + a &= 0 & &\text{\emph{steady state}}
\end{align*}

    \begin{itemize}
    \item $\bn_s = \left<-s_x,-s_y,1\right>$  \, is an upward normal to the ice surface
    \item \emph{not} on the ice: $a\le 0$ for $t>0$ (or in steady state)
    \end{itemize}
\end{itemize}
...
\begin{itemize}
\item the steady IGP free-boundary problem for $s(x,y)$ on $\Omega$
...
\begin{itemize}
\item the evolving IGP free-boundary problem for $s(x,y,t)$ on $\Omega \times [0,T]$
\item consider one step of backward Euler scheme
    \begin{itemize}
    \item solve for $s,\bu,p$ at new time $t^\ell$
    \item $\frac{\partial s}{\partial t} \approx \frac{s^\ell-s^{\ell-1}}{\Delta t}$, and write $s$ for $s^\ell$
    \end{itemize}
\item again: NCP for $s$ coupled to a Stokes problem for $\bu,p$:
...
\end{align*}
    ...
    \begin{itemize}
    \item \alert{pretty similar \dots mostly suffices to consider steady IGP}
    \end{itemize}
\end{itemize}
\end{frame}
...
\item almost no one is solving the implicit IGP
    \begin{itemize}
    \item except Brinkerhoff et al (2017) (\emph{upwinding limits time-step?})
    \item and Wirbel \& Jarosch (2020) (\emph{semi-coupled, artificial ice layer})
    \item neither scalable (\emph{single level})
    \end{itemize}
...
\begin{center}
\mbox{\includegraphics[width=0.7\textwidth]{figs/idoaction.png} \hfill \includegraphics[width=0.2\textwidth]{figs/elephant.png}}
\end{center}
