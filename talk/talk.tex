...
\item this strategy gives $O(m\log m)$ solver for Laplacian obstacle problem (Tai, 2003)
    ...
    \item \only<1>{a multilevel VI strategy is the only hope for (near) optimality}\only<2>{{\color{ForestGreen} a multilevel VI strategy is the only hope for (near) optimality}}
...
\item for IGP a single residual evaluation requires solving Stokes
    \begin{itemize}
    \item expensive but implemented {\Large\strut} \only<2>{{\color{ForestGreen} \, {\Large $\checkmark$}}}
...
\begin{itemize}
\item \only<1>{now all the evil is in the smoother}\only<2>{\alert{now all the evil is in the smoother}}
\item options from weaker to stronger:
    ...
    \item nonlinear pointwise smoothers (GS, Jacobi)?
        \begin{itemize}
        \item[{\color{black} $\circ$}] \only<1>{extremely expensive because residual nonlocal} \only<2>{\alert{extremely expensive because residual nonlocal}}
...
\item \emph{GOAL 1}:\, no shallow approximations in flow physics {\Large \strut} \only<2-3>{{\Large \color{ForestGreen} $\checkmark$}}
    ...
    \end{itemize}
\item \emph{GOAL 2}:\, beat explicit time-stepping and enable paleo {\Large \strut} \only<3>{\Large \alert{:-(}}
    \begin{itemize}
    \item<2-3> problem is
        \begin{itemize}
