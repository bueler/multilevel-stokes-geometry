...
\item $\therefore$ \, $\phi(s)$ is well-defined if $s$ is piecewise $C^1$
    \begin{itemize}
    \item which is a regularity conjecture for the coupled theory
    \end{itemize}
\end{itemize}
...
\item this strategy gives an $O(m\log m)$ solver for the Laplacian obstacle problem (Tai, 2003)
    \begin{itemize}
    \item \only<1>{a multilevel VI strategy is the only hope for (near) optimality}\only<2>{\alert{a multilevel VI strategy is the only hope for (near) optimality}}
    \end{itemize}
\item for IGP:
    \begin{itemize}
